    ...
    <main className="min-h-dvh text-white bg-gradient-to-br from-black to-zinc-900 dark">
      <nav className="px-3 md:px-10 py-3 mb-5 flex justify-between items-center">
        <h4 className="flex items-center text-lg gap-3 font-semibold tracking-tight">
          <IconLayoutKanban className="text-blue-500" /> Timely
        </h4>
        <Link href="https://github.com/bisryy/task">
          <IconBrandGithub />
        </Link>
      </nav>
      ...
      <section className="mb-10 py-5 px-3 md:px-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          <div className="flex flex-col justify-center">
            <h4 className="text-lg font-semibold mb-5 text-blue-300 flex items-center gap-1">
              Task Management made easy!
            </h4>
            <h1 className="text-4xl md:text-5xl xl:text-6xl tracking-tighter font-bold mb-5">
              Plan, Assign, Track and{" "}
              <span className="from-[#1c77ff] to-[#49b8f8] bg-clip-text text-transparent bg-gradient-to-b xl:text-7xl">
                Achieve
              </span>{" "}
              with Timely.
            </h1>
            <p className="text-lg text-zinc-200 mb-5">
              ...
              <span className="text-blue-500">
                perfect <strong>solution</strong>{" "}
              </span>
              to all of your task management need.
            </p>
            <div className="flex gap-5">
              <Button color="primary" as={Link} href="/board">
                <IconRocket />
                Get Started
              </Button>
          ...
          {cardData.map((card, index) => (
            ...
            >
              <CardHeader className="font-bold gap-3">
                <span className="flex items-center justify-center bg-blue-500 rounded-full h-8 w-8 shrink-0 text-black">
                  <card.icon size={20} />
                </span>
                {card.title}
              </CardHeader>
